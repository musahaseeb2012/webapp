...
let remoteBrainAvailable = null; // null = untested, true/false once known
...
function speak(text) {
    ...
    // Prefer a calm British male voice — the classic Jarvis timbre.
    const preferred = voices.find(v => /en-GB/i.test(v.lang) && /male|daniel|george|arthur/i.test(v.name))
        || voices.find(v => /en-GB/i.test(v.lang))
        || voices.find(v => /en[-_]/i.test(v.lang));
    if (preferred) utterance.voice = preferred;

    utterance.onstart = () => reactor.classList.add('speaking');
    utterance.onend = () => reactor.classList.remove('speaking');
    utterance.onerror = () => reactor.classList.remove('speaking');
